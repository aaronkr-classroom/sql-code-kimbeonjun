...
키값 : 가장 단순한 형태의 NoSQL DB. 키를 통해 데이터를 저장/검색. 빠르고 확장성이 좋음
문서 : JSON 또는 BSON 기반으로, 계층적 데이터 저장에 적합. 동적 스키마 제공
열 기반 : 대규모 데이터 분석과 효율적인 저장/검색을 지원. 열 중심으로 설계된 구조
그래프 : 관계와 연결을 중심으로 데이터를 저장. 복잡한 관계 탐색이 핵심
